...
As a wrapper, the file \verb|pages_functions.tex| processes both the different \verb|language_files/XY.tex| files and their corresponding \verb|pages_XY| files. 
Each \verb|language_files/XY.tex| file contains all text passages in their order of appearance on the pages in the corresponding language \verb|XY|, allowing the text to be translated independently of its placement in the document. 
Manual line breaks can also be added here as needed. 
The \verb|pages_XY| files handle the positioning and scaling of the text individually for each language \verb|XY|, thereby accounting for differences in text length between languages. 
The language is selected in \verb|main.tex|; for example, the command \verb|\newcommand{\FDILanguage}{en}| generates the English version. 
At the time of writing, the original German version and the English translation presented here are available.
