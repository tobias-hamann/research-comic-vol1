...
% Uncomment "\scikgtexannoted" if Scikgtex annotations are used
%
%
...
\maketitle
\begin{abstract}
    Comics journalism constitutes a graphic form of literary journalism. According to John S. Bak and Christopher Craig \cite{bak_comics_2026}, comics journalism and literary journalism share key features: they both read like fiction but are based on immersive research, eyewitness accounts, and verifiable facts scrutinized as closely as possible. RWTH Aachen University has recently embraced comics journalism as a means of communicating ongoing research at the Chair of AI Methodology within the Faculty of Computer Science \cite{neuwald_deus_2026}. To enable close observation and immersive documentation of scientific practice, comic artist Alfred Neuwald was invited to work alongside the researchers. The resulting collaboration established the general framework for this project. Here we present the first result of a modified artist-in-residence concept implemented by the German National Research Data Infrastructure for Engineering Sciences (NFDI4ING).
\end{abstract}
...
At RWTH Aachen University, comics journalism has recently been embraced as a means of communicating ongoing research at the Chair of AI Methodology within the Faculty of Computer Science \cite{neuwald_deus_2026}. The collaboration between researchers and comic artist Alfred Neuwald established the general framework for the present project. By combining ethnographic observation with the multimodal representational capacities of comics, we explore comics journalism as a method for documenting situated research practices within the administrative office of the German National Research Data Infrastructure for Engineering Sciences (NFDI4ING).
...
One possibility of achieving this multilingual solution would require Alfred to redraw all texts within the comic in a new language.
...
Then, the original German text written by Alfred was placed on top of the blank background PDF as TikZ nodes (see the \hyperref[comic:de]{German version in the appendix}).
...
To prove this concept, we let \ac{ai} translate the comic into \hyperref[comic:la]{Latin}, \hyperref[comic:elb]{Elvish}, and \hyperref[comic:dra]{Draconic}.
In that manner, all dextrograde languages, i.e. languages that are read from left to right, can be displayed in the comic.
Elvish and draconic furthermore prove, that langugages with non-latin letters can be displayed as well.
...
The source code can be found on \href{https://github.com/tobias-hamann/research-comic-vol1}{GitHub}\footnote{\url{https://github.com/tobias-hamann/research-comic-vol1}} \cite{Hamann_A_FAIR_Comic_2026}.
...
If additional fonts are needed, they are contained in the subfolder \verb|fonts_XY|.
As a wrapper, the file \verb|comic_files\pages_functions.tex| is processing these folders.
...
The language is chosen in the \verb|\comic{XY}| comand. 
It outputs all comic pages in the language \verb|XY| one after another, e.g. \verb|\comic{en}| generates the English version of the comic.
...
The primary comic language and the language-specific title, subtitle, and short title are selected in \verb|main.tex| with \verb|\newcommand{\FDILanguage}{en}|. The article body and the remaining metadata are maintained separately.
As of writing, the original \hyperref[comic:ger]{German} version and this translated \hyperref[comic:eng]{English} version exist along the \hyperref[comic:la]{Latin}, \hyperref[comic:elb]{Elvish}, and \hyperref[comic:dra]{Draconic} versions.
...
% Placeholder for the planned Draconic translation. It deliberately remains
% outside \FDILanguageList until the language and page files are available.
\section{The comic in Draconic (planned)}
\label{comic:dra}
\emph{The rendered Draconic edition is being prepared and will be added in a future version.}
