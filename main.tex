...
Then, the original German text written by Alfred was placed on top of the blank background PDF as TikZ nodes (see the \hyperref[comic:deu]{German version in the appendix}).
...
To prove this concept, we let \ac{ai} translate the comic into \hyperref[comic:lat]{Latin}, \hyperref[comic:qya]{Elvish}, and \hyperref[comic:vss]{Draconic}.
In that manner, all dextrograde languages, i.e. languages that are read from left to right, can be displayed in the comic.
The Elvish edition furthermore demonstrates that languages using non-Latin letters can be displayed as well; the rendered Draconic edition is still being prepared.
...
Every language \verb|CODE| is kept in its own folder \verb|language_files/CODE|, which holds the text file \verb|CODE.tex|, the corresponding folder \verb|pages_CODE| and all language dependent images. Here, \verb|CODE| is the lowercase three-letter identifier from ISO 639 Set 3 (formerly ISO 639-3) \cite{iso_639_2023}, as used by the Zenodo language vocabulary. 
...
If additional fonts are needed, they are contained in the subfolder \verb|fonts_CODE|.
As a wrapper, the file \verb|comic_files\pages_functions.tex| is processing these folders.
Every \verb|language_files/CODE/CODE.tex| contains all text lines ordered by their appearance on the pages in the corresponding language, which allows for a translation of texts independently from their appearance in the document.
...
The \verb|pages_CODE| files handle the positioning and scaling of texts for each language individually, which accounts for different lengths of the same contents in different languages.
...
As of writing, the original \hyperref[comic:deu]{German} version and this translated \hyperref[comic:eng]{English} version exist along the \hyperref[comic:lat]{Latin}, \hyperref[comic:qya]{Elvish}, and \hyperref[comic:vss]{Draconic} versions.
...
% comic_files/pages_functions.tex, aktuell: deu, eng, lat, qya.
% Einzeln geht auch: \FDIComicSection{deu} oder \FDIComicSection[3-5]{deu}
%
\appendix
\FDIAppendixComics

% Placeholder for the planned Draconic translation (Vs'shtak). The
% project-local code vss is not an assigned ISO-639 or Zenodo identifier and
% deliberately remains
% outside \FDILanguageList until the language and page files are available.
\section{The comic in Vs'shtak (Draconic, planned)}
\label{comic:vss}
